...
function NAF_renew_main($args) {
  ...
  ."FROM naf_coach WHERE coachid=".pnVarPrepForStore($uid);
  ...
  if ($expdate == 0) {
    ...
    ."WHERE coachid=".pnVarPrepForStore($uid)." "
    ."  AND action='PAYMENT' "
  ...
  */
  ...
  if ($allowRenew) {
    echo '<form method="post" action="'.pnVarPrepForDisplay('https://www.paypal.com/cgi-bin/webscr').'"><input type="submit" value="Renew Membership" />'
    ...
    .'<input type="hidden" name="return" value="'.pnVarPrepForDisplay(pnGetBaseURL().'transactioncomplete.php').'" />'
    .'<input type="hidden" name="cancel_return" value="'.pnVarPrepForDisplay(pnGetBaseURL()).'" />'
