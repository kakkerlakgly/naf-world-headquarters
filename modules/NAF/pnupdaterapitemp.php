...
function findPreviousRanking($coach, $race, $minId, $minDate) {
  ...
  ."  AND (newdate < '$minDate' OR (newdate='$minDate' AND gameid < $minId)) "
...
function findCoachReputation($coach, $race, $minId, $minDate) {
  ...
  if ($game->EOF) {
    $query = "SELECT ranking FROM naf_coachranking_temp WHERE coachid=$coach AND raceid=$race";
...
function updateNextGame($game, $coach, $race, $newCR) {
  ...
  if ($game->EOF) {
    $query = "UPDATE naf_coachranking_temp "
    ."SET ranking = $newCR "
    ."WHERE coachid=$coach "
    ."  AND raceid=$race";
    $dbconn->Execute($query);
    if ($dbconn->Affected_Rows() == 0) {
      $query = "INSERT INTO naf_coachranking_temp "
...
function deleteGame($gameid) {
  ...
  $query = "SELECT * FROM naf_game_temp WHERE gameid=$gameid";
  $game = $dbconn->Execute($query);
  ...
  $query = "DELETE FROM naf_game_temp WHERE gameid=$gameid";
  $dbconn->Execute($query);
}
...
function updatePreviousRankings() {
  ...
  $query = "SELECT * FROM naf_game_temp WHERE dirty='TRUE'";
  $res = $dbconn->Execute($query);
  ...
  for ( ; !$res->EOF; $res->moveNext() ) {
    ...
    $query = "UPDATE naf_game_temp SET rephome=$prevRank1, repaway=$prevRank2 WHERE gameid=$gameid";
    $dbconn->Execute($query);
...
function updateRaces() {
  ...
  for ( ; !$res->EOF; $res->moveNext() ) {
    ...
    $query = "UPDATE naf_game_temp SET racehome=".$res->Fields('hrace').", raceaway=".$res->Fields('arace')." "
    ."WHERE gameid=".$res->Fields('gameid');
    $dbconn->Execute($query);
    echo " . "; ob_flush(); flush(); // simple progress bar
    echo $dbconn->errorMsg();
  }*/
  $query = "UPDATE naf_game_temp ng, naf_tournamentcoach c SET ng.racehome = c.race WHERE ng.tournamentid=c.naftournament AND ng.homecoachid=c.nafcoach AND ng.racehome=0";
  $dbconn->Execute($query);

  $query = "UPDATE naf_game_temp ng, naf_tournamentcoach c SET ng.raceaway = c.race WHERE ng.tournamentid=c.naftournament AND ng.awaycoachid=c.nafcoach AND ng.raceaway=0";
  $dbconn->Execute($query);
...
function updateRankings() {
  ...
  $query = "UPDATE naf_game_temp SET dirty='FALSE' WHERE gameid=$gameid";
  $dbconn->Execute($query);
...
function updateNewDate() {
  $dbconn =& pnDBGetConn(true);
  $query = "UPDATE naf_game_temp SET newdate = concat( date, ' ', if( HOUR <10, '0', '' ) , HOUR , ':00:00' ) WHERE newdate = '0000-00-00 00:00:00'";
  $dbconn->Execute($query);
}
?>
