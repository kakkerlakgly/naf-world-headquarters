 ...
 ************************************************************************/ 
  ...
  class IPN {
    var $request;    // The full IPN request paypal sent us. Used in validation.
...
//                    "pending_reason=" . urlencode($this->pending_reason) . "&" .
                    ...
                    "payer_id=" . urlencode($this->payer_id) . "&" .
...
//                    "address_country=" . urlencode($this->address_country) . "&" .
      ...
      // Bah. Ugly hack because PayPal can't follow their own specification documents.
      $postFields = $this->request . "&cmd=_notify-validate";

echo "<pre>" . $postFields . "</pre><br />";
      ...
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); // Return the result instead
                                                   // of writing it out.  
      curl_setopt($ch, CURLOPT_POSTFIELDS, $postFields); // Setup post fields.
      ...
      if (strcmp($this->result, "VERIFIED") == 0) {
        return 1;
      }
     ...
     * The variables are taken from the $_POST[] variables.
     */
    function setup($req) {
      $this->request = $req;
      ...
      $this->num_cart_items = $_POST['num_cart_items'];
      $this->payment_status = $_POST['payment_status'];
      $this->pending_reason = $_POST['pending_reason'];
      $this->payment_date = $_POST['payment_date'];
      $this->payment_gross = $_POST['payment_gross'];
      $this->payment_fee = $_POST['payment_fee'];
      ...
      $this->address_street = $_POST['address_street'];
      $this->address_city = $_POST['address_city'];
      $this->address_state = $_POST['address_state'];
      $this->address_zip = $_POST['address_zip'];
      $this->address_country = $_POST['address_country'];
      $this->payment_type = $_POST['payment_type'];
      $this->notify_version = $_POST['notify_version'];
      $this->verify_sign = $_POST['verify_sign'];
      $this->mc_gross = $_POST['mc_gross'];
      $this->mc_fee = $_POST['mc_fee'];
      $this->mc_currency = $_POST['mc_currency'];
    }
    ...
    function logQuery($db, $errorCode=0) {
      ...
      $qry = "INSERT INTO naf_transactions (".
             ...
             " payer_id, payer_email, payer_status, request " .
             ") " .
             "VALUES (" .
             ...
             " '$this->mc_gross', '$this->mc_fee', '$this->mc_currency', '$status', $errorCode, '$this->payer_id', " .
             " '$this->payer_email', '$this->payer_status', '$this->request' ".
             ")";
      if ($db->query($qry) == 0) {
    ...
    function restoreIpn($db, $id) {
      ...
      if ($db->numRows() != 0) {
        ...
        $this->payer_id = $ob->payer_id;
        $this->payer_email = $ob->payer_email;
        $this->payer_status = $ob->payer_status;
