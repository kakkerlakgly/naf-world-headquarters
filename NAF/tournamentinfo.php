<?
pnRedirect(pnModURL('NAF', 'tournamentinfo'));
exit;
  ...
  $uname = pnVarPrepForStore(pnVarCleanFromInput('uname'));
  $tournament = pnVarCleanFromInput('id')+0;

  if ($uid > 0) {
    $query = "SELECT pn_uname FROM nuke_users where pn_uid=$uid";
    $username = $dbconn->getOne($query);
  }
  else {
    $query = "SELECT pn_uid FROM nuke_users where pn_uname='$uname'";
  ...
  if (strlen($username) == 0 || $uid==0)
    ...
    echo "<h3>Tournament info for $username</h3>";
    ...
    for ( ; !$tournaments->EOF; $tournaments->moveNext() ) {
      ...
      if ($tid == $tournament) {
        ...
        $tLink = '<a href="/naf.php?page=tournamentinfo&uid='.$uid.'&id='.$tid.'">'.$tournaments->Fields('tournament').'</a>';
    ...
    if ($tournaments->numRows() > 0) {
      ...
      $query = "SELECT tournamentid, homecoachid, racehome, awaycoachid, goalshome, goalsaway FROM naf_game WHERE homecoachid=$uid";
      ...
      $query = "SELECT tournamentid, awaycoachid, homecoachid, raceaway, goalsaway, goalshome FROM naf_game WHERE awaycoachid=$uid";
      $games = $dbconn->Execute($query);
      ...
      echo '<tr><th bgcolor="#D9D8D0" colspan=\"6\">Tournaments</th></th></tr>';
    ...
    $query = "SELECT name, ranking, naf_race.raceid FROM naf_coachranking LEFT JOIN naf_race USING (raceid) "
            ."WHERE coachid=$uid order by name";
    $rankings = $dbconn->Execute($query);

    if ($rankings->numRows() > 0) {
      echo '<table bgcolor="#858390" cellpadding="2" cellspacing="1" border="0">';
      echo '<tr><th bgcolor="#D9D8D0" colspan=\"5\">Rankings</th></th></tr>';
  ...
  if ($tournament > 0) {
    ...
    $query = "SELECT naf_game.*, u1.pn_uname as homecoachname, u2.pn_uname as awaycoachname, "
            ."r1.name as homeracename, r2.name as awayracename "
            ."FROM naf_game "
            ."LEFT JOIN nuke_users u1 ON (homecoachid = u1.pn_uid) "
            ."LEFT JOIN nuke_users u2 ON (awaycoachid = u2.pn_uid) "
            ."LEFT JOIN naf_race r1 ON (racehome = r1.raceid) "
            ."LEFT JOIN naf_race r2 ON (raceaway = r2.raceid) "
            ."WHERE tournamentid=$tournament order by naf_game.date, naf_game.hour, naf_game.gameid";
    $games = $dbconn->execute($query);
    ...
    if ($games->numRows() > 0) {
      ...
      echo '<table bgcolor="#858390" cellpadding="2" cellspacing="1" border="0">';
      echo '<tr><th bgcolor="#D9D8D0" colspan="6">'.$tName.'</th></th>'
      ...
      for ( ; !$games->EOF; $games->moveNext() ) {
        ...
        if ($games->Fields('homecoachid') == $uid) {
          ...
          $opponent = $games->Fields('awaycoachname');
          ...
          $opponent = $games->Fields('homecoachname');
